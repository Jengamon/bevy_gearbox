...
#[cfg(all(feature = "nightly", compiler_nightly))]
mod transition_event_def {
    use super::*;
    pub trait TransitionEvent: EntityEvent + Clone + 'static {
        type ExitEvent: EntityEvent + Clone = NoEvent;
        type EdgeEvent: EntityEvent + Clone = NoEvent;
...
#[cfg(not(all(feature = "nightly", compiler_nightly)))]
mod transition_event_def {
    use super::*;
    pub trait TransitionEvent: EntityEvent + Clone + 'static {
...
// (The above two modules cover both channels; no third variant needed.)

pub use transition_event_def::Trait as TransitionEvent;
...
pub struct PhaseEvents<Exit: EntityEvent + Clone = NoEvent, Edge: EntityEvent + Clone = NoEvent, Entry: EntityEvent + Clone = NoEvent> {
...
pub trait PhasePayload: Clone + Send + Sync + 'static {
    ...
    fn on_edge(&self, _commands: &mut Commands, _edge: Entity, _children: &Query<&Substates>, _state_machine: &StateMachine) {}
    ...
    for<'a> <Entry as Event>::Trigger<'a>: Default,
{
    ...
    fn on_edge(&self, commands: &mut Commands, _edge: Entity, _children: &Query<&Substates>, _state_machine: &StateMachine) {
...
pub(crate) fn edge_event_listener<E: TransitionEvent + RegisteredTransitionEvent + Clone>(
    ...
    if let Ok(current) = q_sm.get(machine_root) {
        ...
        for &leaf in current.active_leaves.iter() {
            let region_root = find_parallel_region_root(leaf, &q_substate_of, &q_initial_state, &q_children);
...
pub fn check_always_on_guards_changed(
    ...
    for (edge, guards, source, edge_target) in q_guards_changed.iter() {
        ...
        let root = q_substate_of.root_ancestor(source);
        let Ok(sm) = q_sm.get(root) else { continue; };
